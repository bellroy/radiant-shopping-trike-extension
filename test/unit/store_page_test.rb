...
class StorePageTest < Test::Unit::TestCase
  ...
  def test_store_product_each_tag
    ...
    assert_renders '!!!', '<r:shopping:product:each>!</r:shopping:product:each>', '/store/'
  end
  ...
  private
    def create_test_products( number = 1 )
      number.times do |prod_id|
        prod = Product.new(:code => "test-product-#{ prod_id }", :description => "This test product is number #{ prod_id }.", :price => 0.99 + prod_id)
        prod.save!
      end
    end
end
